...
$(".toolbar").delegate(".toobar_information_center","click",function(){
    var sup = $(this).children(".information_num");
    sup.hide();
});

//---------------------系统管理消息弹框关闭--------------------
$(".system_information").delegate("i","click",function(){
    $(this).parents(".system_information").fadeOut();
});

//----------------------模态框相关---------------------
$('.modal').not(".modal_bind_phone").modal({
    keyboard: true,
    backdrop: 'static',
    show:false
});
...
//退出登录
$(".toolopr_log_out").on("click",function(){
    self.location='/Simian/Auth/Login/Logout.html';
})
//
...
$(".modifypassword_form").validate({
    ...
    submitHandler:function(form){
        var old_password=$(".modal_password_old").val();//原密码
        var new_password=$(".modal_password_new").val();//新密码
        // console.log(old_password);
        // console.log(new_password);
        $.ajax({
            type: 'POST',
            url: "/Simian/Auth/Login/Change_password.html",
            data: {
            ...
            success: function (data) {
                if (data == 0) {
                    ...
                    //self.location = '/Simian/Auth/Login/Login.html';
                } else {
                    alert("原密码输入错误");
                }
...
function checkCode(param){
    $(param).delegate('.modal_btn_confirm',"click",function(){
        // console.log(param);
        //modal_phone_solution 手机解绑
        //modal_retrieve_password 修改密码
        var mobile = $(".modal_phone_number:visible").val();
        var vercode = $.trim($(".modal_vercode:visible").val());
        var user_name = $.trim($(".modal_username:visible").val());
        ...
        //if (vercode_val) {
            if(vercode == ""){
                ...
                if(param == ".modal_phone_solution"){
                    $.ajax({
                        type: 'POST',
                        url: "/Simian/Auth/Login/Unbind.html",
                        data: {
                            userphone: mobile,
                            password: vercode
                        },
                        success: function (data) {
                            if (data == 0) {
                                ...
                                $(".modal_tips_vercode").text('*输入号码非注册手机号').show();
                            }else {
                ...
                }else if(param == ".modal_bind_phone"){
                    $.ajax({
                        type: 'POST',
                        url: "/Simian/Auth/Login/Bind.html",
                        data: {
                            userphone: mobile,
                            password: vercode
                        },
                        success: function (data) {
                            if (data == 0) {
                                alert('绑定成功');
                                self.location='/MeeZao/B/User/Welcome';
                            } else if(data == 1){
                                $(".modal_tips_vercode").text('*验证码输入错误').show();
                            }
...
$(function () {
    ...
    (function alertTagRead(){
        ...
        //var alert_tag = $(".content").children(".alert_hide_tag");
        var alert_type = getCookie('tag_type');
        var alert_delay = getCookie('tag_delay');
        var alert_content = decodeURIComponent(getCookie('tag_content'));
        //alert(alert_type);
        //alert(alert_delay);
        //alert(alert_content);
    ...
    function menu_unflod() {
        ...
        if (class_name == "nav_bar_level1") {
            if ($(this).nextAll().is(":hidden")) {
                ...
                $(this).parent().siblings().children("a").removeClass("nav_bar_current1").nextAll().hide();
            }
            else {
    ...
    var initial_path=$(".path_initialize").text().trim();
    // console.log(initial_path);
    initialPath(initial_path);

    function initialPath(path_url){
        $(".nav_bar a").each(function(){
            var url = path_url;
            // console.log(url);
            var a_href=$(this).attr("href");
            // console.log(a_href);
            // console.log(a_href.indexOf(url));
            if(a_href.indexOf(url)>-1){
                $(this).parents("ul").prevAll("a").trigger("click");
                //console.log($(this).parents("ul").prevAll("a").text());
                $(this).css("color","#00b6f6");
                var text_this = $(this).text();
                var closest_ul = $(this).closest("ul");
                if (closest_ul.hasClass("nav_bar_level3")){
                    var text_level2 = $(this).parents(".nav_bar_level3").prevAll("a").find("span").text();
                    var text_level1 = $(this).parents(".nav_bar_level2").prevAll("a").find("span").text();
                    $(".content_route_pathname").text(">"+text_level1+">"+text_level2+">"+text_this);
                }else if(closest_ul.hasClass("nav_bar_level2")){
                    var text_level1 = $(this).parents(".nav_bar_level2").prevAll("a").find("span").text();
                    $(".content_route_pathname").text(">"+text_level1+">"+text_this);
